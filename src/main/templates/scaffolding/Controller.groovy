...
class ${className}Controller {
    ...
    def search ( params, ${className} ${propertyName} ) {
        def result = [: ]

        if ( params.q ) {
            def criteria = ${className}.createCriteria( )
            result.${propertyName}InstanceList = criteria.list( params )
        } else {
            result.${propertyName}InstanceList = ${className}.list( params )
        }
        result.${propertyName}InstanceTotal = ${className}.count( )

        render ( view: "search", model: [ ${propertyName}InstanceTotal: result.${propertyName}InstanceTotal,
            ${propertyName}InstanceList: result.${propertyName}InstanceList, searchText: params.q ] )
    }
